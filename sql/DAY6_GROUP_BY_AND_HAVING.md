...
```sql
SELECT city, course, COUNT(*) AS n
FROM students
GROUP BY city, course WITH ROLLUP;
```
...
| Chennai   | SQL    |  2 |
| Chennai   | Python |  1 |
| Chennai   | NULL   |  3 |  ← Chennai subtotal
| Hyderabad | SQL    |  2 |
| Hyderabad | Python |  1 |
| Hyderabad | NULL   |  3 |  ← Hyderabad subtotal
| Kochi     | SQL    |  2 |
| Kochi     | NULL   |  2 |  ← Kochi subtotal
| NULL      | NULL   |  8 |  ← Grand total
+-----------+--------+----+
```

`WITH ROLLUP` works from **right to left**, removing grouping columns one level at a time.
...
* `Chennai + NULL` → total students in Chennai
* `Hyderabad + NULL` → total students in Hyderabad
* `NULL + NULL` → total students across all cities

> **Note:** The `NULL` values generated by `ROLLUP` represent subtotal/total rows. If the original column can also contain `NULL`, use `GROUPING()` to distinguish them.
...
SELECT city, course, COUNT(*) AS n
FROM students
GROUP BY CUBE(city, course);
```

### Concept

For two columns, `CUBE` produces:
...
| Hyderabad | SQL    |  2 |
| Hyderabad | Python |  1 |
| Chennai   | NULL   |  3 |  ← city subtotal
| Hyderabad | NULL   |  3 |  ← city subtotal
| NULL      | SQL    |  6 |  ← course subtotal
| NULL      | Python |  2 |  ← course subtotal
| NULL      | NULL   |  8 |  ← Grand total
+-----------+--------+----+
```
...
> **Easy way to remember:** `ROLLUP` gives hierarchical subtotals, while `CUBE` gives **all possible subtotal combinations**.

---

# 16. ROLLBACK — Undoing a Transaction

`ROLLBACK` is **not related to `ROLLUP` or `CUBE`**.

`ROLLBACK` is used to **undo changes made during a transaction** before they are committed.
...
CUBE     → GROUP BY + all subtotal combinations
ROLLBACK → Undo transaction changes
COMMIT   → Save transaction changes
```

> **Note:** `ROLLUP`/`CUBE` syntax varies by SQL database. `GROUP BY ... WITH ROLLUP` is commonly used in MySQL, while databases such as PostgreSQL and SQL Server commonly use `GROUP BY ROLLUP(...)`.
